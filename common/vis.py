...
class Visualizer:
    ...
    @classmethod
    def visualize_prediction_batch_v2(cls, 
                                    spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b,
                                    fg_mask_b, bg_mask_b, efg_mask_b, ebg_mask_b,
                                    cls_id_b, batch_idx, iou_b=None, add_fb_mask_to_img=False):
        spt_img_b = utils.to_cpu(spt_img_b)
        spt_mask_b = utils.to_cpu(spt_mask_b)
        qry_img_b = utils.to_cpu(qry_img_b)
        qry_mask_b = utils.to_cpu(qry_mask_b)
        ...
        fg_mask_b = utils.to_cpu(fg_mask_b)
        bg_mask_b = utils.to_cpu(bg_mask_b)
        efg_mask_b = utils.to_cpu(efg_mask_b)
        ebg_mask_b = utils.to_cpu(ebg_mask_b)
        
        cls_id_b = utils.to_cpu(cls_id_b)

        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, fg_mask, bg_mask, efg_mask, ebg_mask, cls_id) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, fg_mask_b, bg_mask_b, efg_mask_b, ebg_mask_b, cls_id_b)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction_v2(spt_img, spt_mask, qry_img, qry_mask, pred_mask,
                                        fg_mask, bg_mask, efg_mask, ebg_mask,
                                        cls_id, batch_idx, sample_idx, True, iou)

    @classmethod
    def visualize_prediction_v2(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask,
                                fg_mask, bg_mask, efg_mask, ebg_mask,
                                cls_id, batch_idx, sample_idx, label, iou=None, add_fb_mask_to_img=False):
    ...
    @classmethod
    def show_fb_mask(cls,fb_mask,qry_img,ratio=0.5):
        pass
