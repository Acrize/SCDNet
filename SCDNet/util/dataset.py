...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None, use_split_coco=False, \
                        ...
                        ms_transform=None, test_num=10000):
        ...
        class_ids = self.sub_list if self.mode=='train' else self.sub_val_list
        self.class_ids = [i - 1 for i in class_ids]

        fss_list_root = os.path.join(data_list, '{}/fss_list/{}/'.format(data_set, mode))
    ...
    def __getitem__(self, index):
        ...
        path = os.path.join(self.base_path,label_path.split('/')[-1])
        ...
        # print('query image:', image.dtype)
        # print('query mask:', label.dtype)
        # print('support image:', s_x.dtype)
        # print('support mask:', s_y.dtype)

        batch = {'query_img': image,
                 'query_mask': label.type(torch.float32),
                 'query_name': image_path,

                 # 'org_query_imsize': org_qry_imsize,

                 'support_imgs': s_x,
                 'support_masks': s_y.type(torch.float32),
                 'support_names': support_image_path_list,
                 'class_id': torch.tensor(class_chosen - 1)}
        ...
        #     return image, label, label_b, s_x, s_y, subcls_list, total_image_list, support_label_list_ori, support_label_list_ori_mask, raw_label, raw_label_b, class_chosen    
